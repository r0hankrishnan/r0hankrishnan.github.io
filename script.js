// Theme variables > reads preferences > fall back to sys > save on toggle
...
// Listen for theme toggle click for both normal and sticky button
btn.addEventListener('click', () => applyTheme(!isDark));
btnSticky.addEventListener('click', () => applyTheme(!isDark));

// Sticky nav > only show when header scrolls off page
...
// Access article RSS feeds from api service and dynamically display
...
// Convert date time to MM YYYY
function formatDate(str) {
    const d = new Date(str);
    return isNaN(d) ? '' : `${MONTHS[d.getMonth()]} ${d.getFullYear()}`;
}

// Fetch articles based on filter selected
